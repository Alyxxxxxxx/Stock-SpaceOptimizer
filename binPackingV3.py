...
def buscaProducto(x, y, x2, y2, alto, diferente = None):
    for i in range( len(productos)):
        ...
        if(auxX*auxY>dimX*dimY):
            ...
            dimY = auxY
        #END if
        if(productos[i] != diferente and productos[i][1] > 0 and productos[i][0].alto <= alto and dimX > 0 and dimY > 0):
            #productos[i][0].color = [random.randrange(0, 3, 1)/2, random.randrange(0, 3, 1)/2, random.randrange(0, 3, 1)/2]
            return productos[i]
        #END if
    #END for
    return None
...
def rellenaPiso(x,y,x2,y2,z,alto):
    global productos, productosAlmacenados, productoActual, productoSiguiente
    materias = []
    altoActual = 0
    if(len(productos) == 0):
        return materias, altoActual
    if(productoActual == None):
        if(productoSiguiente == None):
            productoActual = buscaProducto(x, y, x2, y2, alto)
        else:
            productoActual = productoSiguiente
            productoSiguiente = None
    if(productoActual == None):
        productoActual = productos[0]
    dimX = int((x2 - x) / productoActual[0].largo)
    dimY = int((y2 - y) / productoActual[0].ancho)
    auxX = int((x2 - x) / productoActual[0].ancho)
    auxY = int((y2 - y) / productoActual[0].largo)
    if(auxX*auxY>dimX*dimY):
        aux = productoActual[0].largo
        productoActual[0].largo = productoActual[0].ancho
        productoActual[0].ancho = aux
        dimX = auxX
        dimY = auxY
    producto = None
    if(productoActual[1] > 0 and productoActual[0].alto <= alto and dimX > 0 and dimY > 0):
        producto = productoActual
        
    if(producto == None):
        if(productoSiguiente == None):
            productoSiguiente = buscaProducto(x, y, x2, y2, alto, productoActual)
        if(productoSiguiente != None):
            dimX = int((x2 - x) / productoSiguiente[0].largo)
            dimY = int((y2 - y) / productoSiguiente[0].ancho)
            auxX = int((x2 - x) / productoSiguiente[0].ancho)
            auxY = int((y2 - y) / productoSiguiente[0].largo)
            if(auxX*auxY>dimX*dimY):
                aux = productoSiguiente[0].largo
                productoSiguiente[0].largo = productoSiguiente[0].ancho
                productoSiguiente[0].ancho = aux
                dimX = auxX
                dimY = auxY
            if(productoSiguiente[1] > 0 and productoSiguiente[0].alto <= alto and dimX > 0 and dimY > 0):
                producto = productoSiguiente
    if(producto == None):
...
def binPacking(pro, alto, ancho, largo, espacios, arch, tipoRack): #Productos = [Producto(largo, ancho, alto), cantidad]0
    ...
    nEspacios = 0
    i = 0
    r = math.ceil(len(productos)**(1/3))
    g = math.ceil(((len(productos)/r)**(1/2)))
    b = math.ceil(len(productos) /(r*g))
    colores = []
    for red in range(r):
        for green in range(g):
            for blue in range(b):
                colores.append([1/r * red, 1/g * green, 1/b * blue])
    while(i < len(productos)):
        ...
        if(productos[i][0].alto > altoEspacio):
            ...
            if(productos[i][0].ancho <= altoEspacio):
                productos[i][0].alto = productos[i][0].ancho
                productos[i][0].ancho = aux
            elif(productos[i][0].largo <= altoEspacio):
                productos[i][0].alto = productos[i][0].largo
                productos[i][0].largo = aux
            else:
                productos.pop(i)
        #END if
        else:
            indice = random.randrange(0, len(colores), 1)
            productos[i][0].color = colores[indice]
            colores.pop(indice)
            i += 1
        #END else
    #END while
